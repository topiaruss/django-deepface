...
class TestFaceLoginView:
    ...
    def test_face_login_password_invalid(self, client):
        ...
        assert any('username and password' in str(error) for error in form.errors['__all__'])
...
class TestDeleteFaceView:
    def test_delete_face_success(self, authenticated_client, user, real_face_image, monkeypatch):
    ...
    def test_delete_face_unauthorized(self, client, user, real_face_image, monkeypatch):
        ...
        assert response.status_code == 302  # Redirects to login
        assert Identity.objects.filter(id=identity.id).exists()  # Identity should still exist
