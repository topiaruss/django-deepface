...
def update_badge(version):
    ...
    # Find the PyPI badge line - handle multiple formats
    badge_found = False
    for i, line in enumerate(lines):
        # Check for badge.fury.io format
        ...
        # Check for direct PyPI format
        elif (
            "PyPI version" in line
            and "https://pypi.org/project/django-deepface/" in line
        ):
            print(f"Found PyPI badge at line {i + 1}: {line.strip()}")

            # Extract portions before and after version
            start_idx = line.find("https://pypi.org/project/django-deepface/")
            if start_idx == -1:
                continue
            ...
            new_line = (
                before + f"https://pypi.org/project/django-deepface/{version}/" + after
            )
            lines[i] = new_line
            badge_found = True
            break
